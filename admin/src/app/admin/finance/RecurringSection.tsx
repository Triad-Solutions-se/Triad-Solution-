...
function RecurringFormModal({
  ...
  async function submit(e: React.FormEvent) {
    ...
    const { error } =
      isEdit && initial
        ? await supabase.from("recurring_payments").update(payload).eq("id", initial.id)
  ...
  async function remove() {
    if (!initial) return;
    if (!confirm("Ta bort denna återkommande betalning?")) return;
    setDeleting(true);
    const { error } = await supabase.from("recurring_payments").delete().eq("id", initial.id);
    setDeleting(false);
    if (error) {
      alert(error.message);
      return;
    ...
    <Modal open={open} onClose={onClose}>
      ...
      >
        <h3 className="font-heading text-lg font-semibold">
          {isEdit ? "Redigera återkommande" : "Ny återkommande betalning"}
        </h3>
        <input
          autoFocus
          required

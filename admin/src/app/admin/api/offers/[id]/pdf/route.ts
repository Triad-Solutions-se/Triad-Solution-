...
export async function GET(
  ...
  const buffer = await generateOfferPdf({
    ...
    customer: Array.isArray(offer.customer)
  ...
  const safeName =
    (offer.customer &&
      (Array.isArray(offer.customer)
        ? offer.customer[0]?.name
        : (offer.customer as any).name)) ??
    "kund";
  const filename = `Offert_${offer.offer_number ?? id.slice(0, 8)}_${String(safeName).replace(/[^a-zA-Z0-9_-]/g, "_")}.pdf`;
